...
class Neighborhood(ABC):
    ...
    def selectRandomChromosome(self, solution):
        chromosomes = [f"S{i}" for i in range(1, 9)] # List of chromosomes (S1, S2, ..., S8)
        attr = np.random.choice(chromosomes)  # Select a random chromosome (S1, S2, ..., S8)
        chromosome = getattr(solution, attr) 
        return attr, chromosome
    
    def selectRandomPair(self, chromosome):
        i, j = sorted(np.random.permutation(len(chromosome))[:2]) # Select two random indices i and j
        return i, j
...
class Swap(Neighborhood):
    ...
    def applyChange(self, solution):
        ...
        for _ in range(self.N):
            ...
            chromosome[i], chromosome[j] = chromosome[j], chromosome[i]  # Swap the elements at indices i and j
            setattr(solution_copy, chromosome_attr, chromosome)  # Update the solution with the modified chromosome
...
class Reversion(Neighborhood):
    ...
    def applyChange(self, solution):
        ...
        for _ in range(self.N):
            ...
            chromosome[start:end] = chromosome[start:end][::-1]  # Reverse the selected portion of the chromosome
...
class Insertion(Neighborhood):
    ...
    def applyChange(self, solution):
        ...
        for _ in range(self.N):
            ...
            chromosome[i + 1] = unit_to_insert # Insert the unit in its new position
            setattr(solution_copy, chromosome_attr, chromosome)
...
class Slide(Neighborhood):
    ...
    def applyChange(self, solution):
        ...
        for _ in range(self.N):
            ...
            chromosome[j] = unit_to_move  # Place the moved unit in its new position
            setattr(solution_copy, chromosome_attr, chromosome)
...
class ETN(Neighborhood):
    ...
    def applyChange(self, solution):
        ...
        for _ in range(self.N):
            chromosome_attr, chromosome = self.selectRandomChromosome(solution_copy)
            i = np.random.randint(0, len(chromosome) - 1)  # Select a random index within the valid range
...
class RS(Neighborhood):
    ...
    def applyChange(self, solution):
        ...
        for _ in range(self.N):
            chromosome_attr, chromosome = self.selectRandomChromosome(solution_copy)
            start, end = sorted(np.random.choice(len(chromosome), 2, replace=False)) # Select start and end indices for the subsequence
            subsequence = chromosome[start:end+1] # Extract the subsequence
            np.random.shuffle(subsequence) # Shuffle the subsequence randomly
            chromosome[start:end+1] = subsequence # Place the shuffled subsequence back into the chromosome
            setattr(solution_copy, chromosome_attr, chromosome)
...
class SPS(Neighborhood):
    ...
    def applyChange(self, solution):
        ...
        for _ in range(self.N):
            chromosome_attr, chromosome = self.selectRandomChromosome(solution_copy)
            start1, end1 = sorted(np.random.choice(len(chromosome), 2, replace=False)) # Select start and end indices for the first subsequence
            new_position = np.random.randint(0, len(chromosome) - (end1 - start1)) # Select the new position for the first subsequence
            subsequence1 = chromosome[start1:end1+1] # Extract the first subsequence
            
            # Remove the first subsequence from the chromosome
            remaining_indices = np.concatenate((np.arange(start1), np.arange(end1+1, len(chromosome))))
