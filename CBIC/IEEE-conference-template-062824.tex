...
\author{\IEEEauthorblockN{Raissa Gon\c{c}alves Diniz}
\IEEEauthorblockA{\textit{Operations Research and Complex Systems Laboratory} \\
\textit{Universidade Federal de Minas Gerais}\\
Belo Horizonte, Brasil \\
raissagd@ufmg.br}
\and
\IEEEauthorblockN{Andr\'e Costa Batista}
...
\begin{abstract}
    ...
    cadeias, a maioria dos estudos concentra-se no uso de algoritmos populacionais para enfrentar esses desafios. Neste trabalho, apresenta-se a aplicação do algoritmo de 
    Busca em Vizinhança Variável (\textit{Variable Neighborhood Search - VNS}) e avaliamos seu desempenho em comparação com uma metaheurística populacional, especificamente 
    o Algoritmo Genético (\textit{Genetic Algorithm - GA}). Os resultados mostram que o VNS apresenta consistentemente um desempenho superior ao GA, tanto em termos de 
    eficiência computacional quanto de qualidade das soluções, especialmente em instâncias de maior porte. Esses achados destacam o potencial do método proposto como uma 
    abordagem eficaz para a otimização de cadeias de suprimentos.
\end{abstract}
...
de otimização baseadas em Algoritmos Genéticos (\textit{Genetic Algorithm - GA}) e outras meta-heurísticas evolucionárias para a logística do pistache. No entanto, muitas dessas comparações se baseiam apenas 
...
Neste estudo, propõe-se a otimização da cadeia de suprimentos do pistache utilizando a metaheurística Busca em Vizinhança Variável (\textit{Variable Neighborhood Search - VNS}), que é amplamente reconhecida 
...
\begin{figure}
  \begin{center}
      \includegraphics[width=9cm]{./imgs/fluxo_pt.PNG}
...
A abordagem proposta inclui operadores de vizinhança personalizados e configurações de parâmetros adequadas.

\subsection{Representação da Solução}
Seguindo \cite{b2} e \cite{b12}, é utilizada a codificação baseada em prioridade para representar as soluções. Cada par origem-destino é tratado como um gene, cujas prioridades 
...
Diferentemente de \cite{b2}, que utiliza codificação real com valores contínuos entre 0 e 1, emprega-se uma abordagem baseada em números inteiros sequenciais de 1 a $Z$, onde $Z$ representa o número total 
de elementos considerados na ordenação de prioridades para um dado fluxo logístico (por exemplo, a soma entre fontes e destinos de transporte em determinado segmento da cadeia). Essa representação ordinal, 
atribuída com base nos custos médios de transporte, visa aprimorar a interpretabilidade das soluções e facilitar a implementação de operadores de vizinhança. Além disso, a codificação inteira permite uma 
distinção mais clara entre soluções vizinhas, o que se mostra vantajoso em problemas combinatórios, como é o caso da otimização de cadeias de suprimentos. A eficácia desse tipo de codificação já foi 
demonstrada em estudos anteriores (\hspace{-0.03em}\cite{b11,b12,b13}).

A Figura~\ref{fig:cromossomo} apresenta a estrutura do cromossomo adotado, composto por oito segmentos ($S_1$ a $S_8$), cada um responsável por ordenar as prioridades 
em um fluxo logístico específico da cadeia de suprimentos. A parte inferior da figura ilustra um exemplo de cromossomo completo, com prioridades inteiras associadas a 
cada par origem-destino. Essa estrutura modular facilita a aplicação de operadores de vizinhança e o processo de decodificação baseado em transporte prioritário.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{./imgs/chromo.PNG}
    \caption{Representação da estrutura do cromossomo baseado em prioridade. A parte inferior mostra um exemplo de um cromossomo preenchido com as prioridades inteiras.}
...
\begin{figure}[h!]
  \begin{center}
  \includegraphics[width=9cm]{./imgs/decode_flux.PNG}
  \caption{Etapas de decodificação do fluxo na cadeia de suprimentos}
  \label{fig:decode_flux}
  \end{center}
\end{figure}

Para a decodificação de cada fluxo logístico da cadeia de suprimentos, utiliza-se a função \textit{decodingStep}, que implementa um algoritmo de transporte baseado em prioridades, conforme apresentado 
no \textit{Procedure 1} de \cite{b12}. Essa função atua iterativamente, alocando unidades de produto de uma fonte para um destino com base em um vetor cromossômico que define a ordem de prioridade entre 
os nós logísticos, sempre respeitando as capacidades das fontes e as demandas dos destinos, além dos custos unitários de transporte. Os Algoritmos~\ref{algoDecode1} e~\ref{algoDecode2} detalham o processo 
completo de decodificação de uma solução. O Algoritmo~\ref{algoDecode1} inicia o processo ao decodificar os fluxos principais entre fábricas, centros de extração, consumidores e centros de processamento, 
utilizando os segmentos cromossômicos $S_1$ a $S_5$. Já o Algoritmo~\ref{algoDecode2} realiza ajustes adicionais nos fluxos para garantir a coerência entre entradas e saídas nos centros intermediários, 
decodifica o transporte dos produtores para os centros de processamento ($S_6$), aloca os resíduos gerados nos processos para centros de compostagem ($S_7$ e $S_8$), e determina quais instalações devem 
ser efetivamente abertas com base nos fluxos obtidos. Ao final do processo, obtém-se um conjunto completo de variáveis de decisão contínuas e binárias que representam uma solução viável e coerente para 
o problema de otimização proposto.
...
com vetores selecionados aleatoriamente, uma população de 100 indivíduos e uma estratégia de seleção por Torneio Binário. Além disso, seus outros principais parâmetros foram definidos com base em valores tradicionais da literatura: 
taxa de cruzamento de 0,8 e taxa de mutação de 0,2. O processo de inicialização utilizou geração estocástica dos cromossomos.

Tanto o VNS quanto o GA foram executados 30 vezes por instância. O critério de parada adotado foi um número máximo de avaliações da função objetivo, definido empiricamente 
a partir de testes sucessivos, de forma a equilibrar custo computacional e qualidade das soluções. Especificamente, foram utilizadas 6.250 avaliações para instâncias de 
tamanho 100; 12.500 para 200; 25.000 para 400; 50.000 para 800; e 100.000 para 1.600. Adicionalmente, o VNS também interrompe a execução caso todos os operadores de 
vizinhança falhem em melhorar a solução atual.
...
\begin{figure*}[h!]
  \centering
  \begin{tabular}{ccc}
    \includegraphics[width=0.30\textwidth]{./imgs/GAxVNS_100.png} &
    \includegraphics[width=0.30\textwidth]{./imgs/GAxVNS_200.png} &
    \includegraphics[width=0.30\textwidth]{./imgs/GAxVNS_400.png} \\
    (a) $I, J, K, E, S, Q, N1, N2, N3, M = 100$ &
    (b) $= 200$ &
    (c) $= 400$ \\
    \multicolumn{3}{c}{\vspace{0.8em}} \\
    \multicolumn{3}{c}{
      \begin{tabular}{cc}
        \includegraphics[width=0.30\textwidth]{./imgs/GAxVNS_800.png} &
        \includegraphics[width=0.30\textwidth]{./imgs/GAxVNS_1600.png} \\
        (d) $= 800$ &
        (e) $= 1600$
      \end{tabular}
    }
...
de Programação Linear Inteira Mista (MILP), ele é resolvido por meio do algoritmo VNS, que explora o espaço de soluções utilizando estruturas de vizinhança como \textit{Swap}, \textit{Reversion}, \textit{Insertion} e \textit{Slide}.
...
Reconhece-se que a comparação entre os algoritmos pode ser influenciada pelos critérios de parada adotados. Embora ambos os métodos utilizem o mesmo número máximo de avaliações da função 
objetivo como critério principal, o VNS possui um critério adicional de parada antecipada quando todos os operadores de vizinhança falham em melhorar a solução atual consecutivamente. 
Essa diferença metodológica pode contribuir para os menores tempos de execução observados no VNS, sugerindo maior eficiência na convergência. Como trabalho futuro, recomenda-se a 
investigação de critérios de parada unificados para permitir comparações ainda mais rigorosas entre as abordagens.

Como perspectiva de melhoria adicional, pesquisas futuras podem explorar operadores de vizinhança específicos para o problema, aprimorando ainda mais o desempenho do VNS. Além disso, uma abordagem híbrida, combinando VNS com métodos 
