...
\abstract{Supply chains are vital for the agricultural sector, particularly for high-value crops like pistachios, where efficient management can enhance both 
economic growth and sustainability. Although there has been research on optimizing pistachio supply chains, most studies rely heavily on population-based 
algorithms to solve these problems. In this study, we propose to apply a Variable Neighborhood Search (VNS) algorithm and comparing its performance 
against a population-based Genetic Algorithm (GA) and an exact method (Gurobi). Our findings reveal that VNS consistently outperforms GA in both execution 
time and objective function value, particularly for larger instances. Additionally, VNS reduces execution time by nearly 97\% compared to the exact algorithm 
in some of the larger instances, making it a highly efficient solution for pistachio supply chain optimization.
}
...
their high economic and nutritional value. As major producers such as the US, Turkey, and Iran lead global production, leading producers of pistachios
worldwide in 2022 and 2023 \cite{statista2023pistachio}, the strategic importance of pistachios in international trade continues to rise.
...
In related research, \cite{rajabi2023utilizing} worked on optimizing the pistachio supply chain using population-based algorithms, 
such as Genetic Algorithms (GA) (\cite{fathollahi2018stochastic}, \cite{xie2022adaptive}). While their approach was effective, our study works further on 
it upon it by employing the Variable Neighborhood Search (VNS) algorithm. VNS is particularly well-suited for larger and more complex problems due to its ability to 
dynamically explore different neighborhood structures, offering more robust diversification and intensification strategies compared to GA. 
Unlike population-based algorithms that rely heavily on crossover and mutation, VNS avoids issues like premature convergence by flexibly 
exploring local optima, resulting in higher-quality solutions in less time, especially for large-scale networks like the pistachio supply chain.
...
designing an optimal supply chain network for pistachios by leveraging VNS. We propose a mixed linear model that integrates forward and backward 
...
we apply the Variable Neighborhood Search (VNS) algorithm and compare its effectiveness with an Exact Algorithm and a Genetic Algorithm. Our 
findings underscore VNS’s superior performance in handling larger problem sizes, demonstrating its potential as a scalable and efficient tool 
for sustainable supply chain management in agriculture.
...
structures, and algorithmic details. In Section~\ref{sec:sec4}, we detail the Results of our experiments, comparing VNS with both an exact algorithm (Gurobi) 
and a population-based algorithm (Genetic Algorithm), highlighting VNS’s superior performance in larger instances. Finally, Section~\ref{sec:sec5} presents 
...
The proposed network, as illustrated in Figure~\ref{fig:supplychain} includes producers, processing centers, pistachio factories, 
...
distribution. Notably, this is not a closed-loop supply chain (\cite{pishvaee2010memetic}, \cite{ahmadi2019integrated}, \cite{banasik2016simple}, 
