...
This omission hinders reproducibility and obscures the mechanism by which constraints are handled. Furthermore, recent works have raised serious concerns about so‑called 
metaphor‑based metaheuristics, arguing that many of these algorithms offer little genuine algorithmic novelty beyond a change of terminology and that their performance 
is often comparable to, or worse than, that of well‑established methods (\cite{campelo2023, aranha2022}). In this context, algorithms such as the Imperialist Competitive Algorithm and the Social 
Engineering Optimizer exemplify a broader trend where new metaphors are introduced without clear benefits over classical metaheuristics, despite adding conceptual complexity.
...
Algorithm \ref{algoDecode1} details the first phase of this process, utilizing the helper function \textit{decodingStep} (Algorithm \ref{algoDecodingStep}) to allocate flows. 

The critical logic occurs in Steps 5 and 6. Here, the algorithm reconciles diverging flows from Processing Centers, which produce multiple 
outputs (Kernels for Oil vs. Open-shell for Factories) in fixed proportions. Satisfying one downstream demand often creates surplus of the other output. The decoding 
identifies the bottleneck demand and routes surplus to the lowest-cost destination, minimizing over-supply penalties.

Each segment of the chromosome ($S_1$ through $S_8$) is decoded using the priority-based procedure described by \citet{gen2006}, which allocates transportation flows 
by iterating through facilities in descending priority order. At each step, the algorithm assigns available capacity from sources to destinations with remaining demand, 
selecting the minimum-cost arc. This greedy strategy ensures feasible solutions while respecting all capacity and demand constraints. The complete decoding procedure 
is detailed in Table 2 of \citet{gen2006}.
...
structure, with a minimum threshold of 10 perturbations to ensure meaningful changes even in small instances. The specific operator used for shaking changes cyclically, 
resetting to the first operator upon a successful acceptance and moving to the next operator if the candidate solution is rejected.
...
This specific ordering follows a strategic progression from conservative to aggressive exploration. First, Active Reversion fine-tunes the existing solution topology by reordering active nodes—this 
intensification step exploits the current structure before considering more disruptive changes. Second, Inactive-Active Swap introduces global diversification by randomly exchanging nodes between 
...
structures against standard operators. Second, we benchmark the optimized VNS against a Genetic Algorithm (GA), Iterated Local Search (ILS), and the exact solver Gurobi.
...
We performed a statistical validation of the results across all instance sizes. First, the Shapiro-Wilk test was applied to check for normality in the distribution of objective 
...
\begin{figure*}[htbp]
    ...
    \caption{Distribution of objective function values for Proposed VNS (Novel) vs. Standard VNS (Basic) across all instance sizes. The novel approach demonstrates superior scalability.}
...
To rigorously validate the performance differences, we applied the same statistical methodology described in Section~\ref{subsec:operators_analysis}. 
...
between active and inactive nodes, significantly outperform standard permutation moves.
...
Regarding future research, two primary directions emerge. First, further variations of the problem model should be explored, such as incorporating explicit closed-loop feedback loops or 
stochastic parameters to better reflect agricultural uncertainties. Second, and critically, there is a need to investigate new encoding methodologies specifically designed to enable 
matheuristics. While priority-based encoding is effective for pure metaheuristics, developing representations that align more closely with mathematical programming structures could 
unlock powerful hybrid decomposition strategies. Bridging the gap between heuristic flexibility and the precision of exact solvers remains a vital frontier 
for solving such complex, large-scale supply chain problems.
